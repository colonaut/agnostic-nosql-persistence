...
const analyzeQuery = (query, schema_analyzer) => {
    ...
    Object.keys(query).forEach((key) => {
        ...
        if (typeof value === 'string') {
            ...
            result.array = approximately = value;
        }
...
const QueryAnalyzer = function (query, schema_analyzer) {
    ...
    const approximate_query = analyzed_query.approximately;
    const approximate_values = keys.map(key => approximate_query[key]);


    //TODO determinie if it is an indexed query OR directly throw if not


    this.approximateQuery = () => approximate_query;

    this.approximateValues = () => approximate_values;
