...
const queryError = (message, value, callback) => callback(new Error(message), false);
...
//TODO: why do i have blockings here?
...
const Query = function (query, schema_analyzer) {
    const keys = Object.keys(query).filter(key => schema_analyzer.index().includes(key)); //TODO: rather expose error instead of filently filter non indexed! do it in match?
