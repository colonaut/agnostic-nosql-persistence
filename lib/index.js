...
module.exports = function (schema, index, model_name, options, callback) {
    if (typeof callback === 'function') { //TODO callback way
        resolveSchema(schema, {index: index}, (err, resolved_schema) => {
            ...
            return callback(null, new PersistenceModel(resolved_schema, model_name, options));
        });
    }

    return resolveSchema(schema, {index: index}).then((resolved_schema) => {
        return Promise.resolve(new PersistenceModel(resolved_schema, model_name, options));
    }).catch((err) => {
