...
const Model = function (schema, index, model_name, options) {
    ...
    const resolved_schema = resolveSchema(schema, {index: index}); //TODO: event loop!


    console.log('???', resolved_schema, resolved_schema.keys);
     ...
     * @returns {string}
     */
    this.getIndexKey = (data) => { //TODO: with callback? event loop! and maybe better in resolved-schema
    ...
    this.find = (query, callback) => {
        resolveQuery(query, resolved_schema)
            .then((analyzed_query) => {
                adapter().find(analyzed_query, callback);
            });
    };
