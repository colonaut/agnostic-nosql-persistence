...
function Model(resolved_schema, index, model_name, options) {
    ...
    const adapter = () => {
        if (!adapter_instance) {
            let _adapter = options.adapter;
            _adapter = _adapter === 'function' ? _adapter
     ...
     * //lorum ipsum
     */
    this.getIndexKey = (data, callback) => { //TODO: with callback! event loop! and maybe better in resolved-schema. get rid of return and then put it to resolved_schema
        let index_key = model_name + options.id_separator;
        let index_value;

        const process = (items) => {
            index_value = data[items.shift()] || 'null';
            ...
            //return index_key; //TODO: remove once all adapters use callback
        };
        process([].concat(index));
    };
