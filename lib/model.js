...
class Model {
    ...
    get adapter(){
        if (!this._adapter_instance) {
            ...
            this._adapter_instance = new adapter(this.getIndexId.bind(this), this.schemaDefinition, options);
    ...
    get schemaDefinition(){
        if(!this._schema_definition) {
            ...
            this._schema_definition = Object.assign({}, shallow_schema_desc);
            for (let prop in shallow_schema_desc) {
                let schema_index = this._index.indexOf(prop);
                if (schema_index !== -1)
                    this._schema_definition[prop].index = schema_index + 1;
            }
        }
