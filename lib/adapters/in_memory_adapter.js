    ...
    this.find = (query_analyzer, callback) => { //TODO: implement functions as query value
        ...
        Object.keys(DATA[model_name]).forEach((index_key) => {
            ...
            if (query_values.every(qv => String(index_key).includes(qv))) {
                console.log('-> approximate match:', index_key, ' <-> ', query_index_key, 'query values:', query_values);
                //check exact match
                let match = DATA[model_name][index_key];
                if (query_analyzer.keys().every((qk) => {
                        console.log('?->', match[qk], 'vs.', query_analyzer.value(qk));
                        let match_as_array = [].concat(match[qk]).sort();

                        //query item has exact match (for arrays, one must contain the exact string). if query was string, only one item is in array, which fullfills the query or not
                        if (match_as_array.some(mi => mi === query_analyzer.value(qk))) {
                            console.log('--> full match for:', qk);
                            return true;
                        }
                        //starts with check: for string or array of strings, whereas numbers are converted.
                        if (match_as_array.some(mi => String(mi).startsWith(query_analyzer.left(qk)))) { //would work with arrays but we ind array e must do it for every item....
                            console.log('--> left match for:', qk);
                            return true;
                        }
                        //query is an array, check if it has an exact match on the matched item
                        if (String(query_analyzer.array(qk)) === String(match_as_array)) {
                            console.log('--> full array match for:', qk);
                            return true;
                        }

                    })) {
                    console.log('---> evaluated match, query key', query_index_key, 'matches', index_key);
                    result.push(match);
                }
            }
        });
