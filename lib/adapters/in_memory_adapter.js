...
const InMemoryAdapter = function(getIndexKey, schema_analyzer, options) {
    ...
    this.find = (query_analyzer, callback) => { //TODO: implement functions as query value
        ...
        let query_index_key = getIndexKey(query_analyzer.approximateQuery());
        //query values array
        let query_values = query_analyzer.approximateValues(); //it also supports nullable index keys.. if they can be nullable...



        console.log('---------------------------------------------')
        console.log('query values', query_values, 'qu');

        Object.keys(DATA[model_name]).forEach((index_key) => {
            //console.log(index_key, ' <-> ', query_index_key, query_index_values);

            //a direct match
            if (index_key === query_index_key) {
                console.log('-> direct match:', index_key, ' <-> ', query_index_key, query_values);
                return result.push(DATA[model_name][index_key]);
            }

            //TODO: null werte und modelname raus, dann any statt every.

            if (query_values.every(qp => String(index_key).includes(qp))) {
                //a match
                console.log('-> a match:', index_key, ' <-> ', query_index_key, query_values)
