...
const InMemoryAdapter = function(getIndexId, schema_analyzer, options) {
    ...
    this.find = (query, callback) => { //TODO: open for search with incomplete query values... but find a way to flag it within the query object
        let result = [];
        //approximation
        let query_values = getIndexId(query).split('~null'); //it also supports nullable index keys.. if they can be nullable...
