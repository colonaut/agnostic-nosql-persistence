...
class CassandraAdapter {
    ...
    drop(callback) { //TODO: implement recreate!, adapt in all adapters (we need it here, as table is gone! for tests an immediate recreation is neccessary)
        let table_query = "DROP TABLE IF EXISTS " + this._model_name;
        console.log('<-', table_query);
        this._client.execute(table_query, {}, {prepare: true}, (err, res) => {
            if (err)
                return callback(err);

            return callback(null);
            //we propably do not need to drop indexes
            console.log(err, res);
            let index = this._getSchemaDetails().index;
            const dropIndex = (i) => {
                if (i >= index.length)
                ...
                let index_query = "DROP INDEX " + this._model_name + "_" + index[i];
                console.log('<-', index_query);
                this._client.execute(index_query, {}, {prepare: true}, (err, res) => {
                    console.log(err, res);
                    if (err)
