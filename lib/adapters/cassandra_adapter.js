...
const CassandraAdapter = function (getIndexId, resolved_schema, options) {
    options = options || {};
    ...
    const getIndexDefinition = (col_name) => { //TODO check the right types like in create table
        if (resolved_schema.value(col_name).typeOf('array'))
    ...
    const getQueryParts = (model) => {
        ...
        resolved_schema.values().forEach((schema_item) => {
    ...
    const ensureIndex = () => {
        return new Promise((resolve, reject) => {
            ...
            indices = [];
            Async.every(resolved_schema.index(), (index_item, callback) => {
    ...
    const ensureTable = () => {
        return new Promise((resolve, reject) => {
            ...
            const getColumnDefinition = (col_name) => {
                ...
                let schema_item = resolved_schema.value(col_name);
                if (schema_item.typeOf('array')) {
                    if (resolved_schema.index().indexOf(col_name) > -1)
    ...
    this.find = (query, callback) => {
        ...
        client.eachRow(cql, {}, {prepare: true}, (n, row) => {
            query.match(row, (err, res) => {
                ...
                //TODO: how to get error/s) in callback below?
