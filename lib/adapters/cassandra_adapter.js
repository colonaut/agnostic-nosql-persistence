...
const CassandraAdapter = function (getIndexId, schema_analyzer, options) {
    ...
    this.find = (query_analyzer, callback) => {
        //console.log('qa', query_analyzer);

        const startsWithQueryValue = (query_value, row_value) => {
            console.log('<-- find exact match', row_value.startsWith(query_value), row_value, 'starts with', query_value);
            return row_value.startsWith(query_value);
        };

        let query_parts = [];
        let row_methods = {};
        query_analyzer.keys().forEach((key) => {
            let array = query_analyzer.array(key);
            let left = query_analyzer.left(key);
            let number = query_analyzer.number(key);
            let rm = [];
            let qp;

            if (array) {
                qp = `${key} CONTAINS '${array.join(`' AND ${key} CONTAINS '`)}'`; //array always is "contains". //TODO: numbers
            } else if (number) {
                if (left)
                    qp = `${key} <= ${query_analyzer.number(key)}`;
                else
                    qp = `${key} = ${query_analyzer.number(key)}`;
            } else {
                if (left)
                    rm = query_analyzer.startsWith; //TODO: we have to restructure that. we might go for approximations... or we need in query_analyzer something like 'has expression' or the expression is empty or filled... so we can ask for expression here
                else
                    qp = `${key} = '${query_analyzer.value(key)}'`;
            }
        ...
        let query = `SELECT * FROM  ${model_name}${where}`; //TODO: allow filtering slows down, but gives most possibilities.
        console.log('<- approximate find query', query);

        let models = [];
        client.eachRow(query, {}, {prepare: true}, (n, row) => {
            let is_match = true;
            row.keys().forEach((key) => {
                if (is_match && row_methods[key])
                    is_match = row_methods[key](row.get(key), key); //TODO: key could be wrong, when row key is nor index key and therefore query key.. (i.e. when we hae to give the key another name, i.e. _xxx does not work. Whe have to take care of that)
            });
