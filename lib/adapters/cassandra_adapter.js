    ...
    this.find = (query_analyzer, callback) => {
        ...
        query_analyzer.keys().forEach((key) => {
            ...
            if (array)
                ...
                qp = `${key} CONTAINS '${array.join(`' AND ${key} CONTAINS '`)}'`;
            else
                qp = `${key} = '${query_analyzer.value(key)}'`;

            query_parts.push(qp);
        });
        //console.log(query_parts.join(' AND '));

        let query = `SELECT * FROM  ${model_name} WHERE ${query_parts.join(' AND ')} ALLOW FILTERING`;
        console.log(query);
        ...
        client.execute(query, {}, {prepare: true}, (err, res) => {
            ...
            let results = res.rows.map(row => {
                //TODO: here we do things after approximation! (if neccessary)
                let model = Object.assign({
                    _id: row.values()[0]
                }, row);
                delete model['partition_key'];
